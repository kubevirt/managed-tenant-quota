...
export MTQ_NAMESPACE="mtq-$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 5 | head -n 1)"

echo "mtq-namespace: ${MTQ_NAMESPACE}"

if [[ -n "$RANDOM_CR" ]]; then
  export CR_NAME="${MTQ_NAMESPACE}"
fi
...
export MTQ_NAMESPACE="${MTQ_NAMESPACE:-mtq}"
