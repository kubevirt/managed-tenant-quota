...
const (
	annCertConfig = "operator.mtq.kubevirt.io/certConfig"
)

// CertManager is the client interface to the certificate manager/refresher
type CertManager interface {
	Sync(certs []mtqcerts.CertificateDefinition) error
}
...
func (cm *certManager) Sync(certs []mtqcerts.CertificateDefinition) error {
...
func (cm *certManager) ensureSigner(cd mtqcerts.CertificateDefinition) (*crypto.CA, error) {
...
func (cm *certManager) ensureCertConfig(secret *corev1.Secret, certConfig mtqcerts.CertificateConfig) (*corev1.Secret, error) {
...
func (cm *certManager) ensureCertBundle(cd mtqcerts.CertificateDefinition, ca *crypto.CA) ([]*x509.Certificate, error) {
...
func (cm *certManager) ensureTarget(cd mtqcerts.CertificateDefinition, ca *crypto.CA, bundle []*x509.Certificate) error {
	listers, ok := cm.listerMap[cd.SignerSecret.Namespace]
	if !ok {
